...
### 2. Install profiling tools and compile kernel BPF samples
```
sudo sh install_server.sh 3
```

### 3. Setup RSS, IRQ, CPU frequency
...
### 1. clone repository and update machine config file
```
...
### 3. (Optional) Check whether TRex is installed
Start TRex server
```
...
## 3. Run experiments
### 1. Set up ssh from the server (node-0) to the client (node-1)
...
### 2. Compile BPF benchmarks on node-0
```
cd ~; sudo sh ~/bpf-profile/samples/compile.sh
```

### 3. Preparations for experiments on node-0
```
cd ~; sh ~/bpf-profile/experiment_prepare.sh
```

### 4. Run experiments on node-0
```
cd ~/experiment
