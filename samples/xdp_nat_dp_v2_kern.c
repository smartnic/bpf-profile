...
static inline void process_metadata(struct xdp_md *ctx) {
  ...
  for (int i = 0; i < NUM_PKTS - 1; i++) {
    ...
    if (md->eth.proto != htons(ETH_P_IP)) {
    ...
    bpf_printk("Processing IP packet: src %04x, dst: %04x", ntohl(md->ip.saddr),
               ntohl(md->ip.daddr));
    srcIp = md->ip.saddr;
    dstIp = md->ip.daddr;
    proto = md->ip.protocol;
    if (md->ip.protocol != IPPROTO_UDP) {
      bpf_printk("Received Packet is not UDP Packet");
      continue;
    }
    bpf_printk("Packet is UDP: src_port %d, dst_port %d",
               ntohs(md->udp.source), ntohs(md->udp.dest));
    srcPort = md->udp.source;
    dstPort = md->udp.dest;
