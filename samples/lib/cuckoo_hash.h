...
#define BPF_CUCKOO_HASH(_name, _key_type, _leaf_type, _max_entries)                                \
    ...
    static int _name##_cuckoo_insert_loop(uint32_t index, void *data) {                            \
        ...
        struct _name##_cuckoo_hash_cell x, tmp;                                                    \
                                                                                                   \
        memset_cilium(&x, 0, sizeof(struct _name##_cuckoo_hash_cell));                                    \
        memset_cilium(&tmp, 0, sizeof(struct _name##_cuckoo_hash_cell));                                  \
                                                                                                   \
        memcpy_cilium(&x, &ctx->new_elem, sizeof(struct _name##_cuckoo_hash_cell));                       \
                                                                                                   \
        ...
        memcpy_cilium(&tmp, elem, sizeof(struct _name##_cuckoo_hash_cell));                               \
                                                                                                   \
        memcpy_cilium(elem, &x, sizeof(struct _name##_cuckoo_hash_cell));                                 \
                                                                                                   \
        if (!tmp.is_filled) {                                                                      \
            ...
            return 1;                                                                              \
                                                                                                   ...
                                                                                                   \
        memcpy_cilium(&x, &tmp, sizeof(struct _name##_cuckoo_hash_cell));                                 \
        ...
        if (!tmp.is_filled) {                                                                      \
            ...
            return 1;                                                                              \
                                                                                                   ...
                                                                                                   \
        memcpy_cilium(&ctx->new_elem, &tmp, sizeof(struct _name##_cuckoo_hash_cell));                     \
    ...
    static __always_inline bool _name##_cuckoo_insert(struct _name##_cuckoo_hash_map *map,         \
        ...
        if (elem->is_filled) {                                                                     \
            if (memcmp_cilium(key, &(elem->key), sizeof(_name##_cuckoo_key_t)) == 0) {                    \
                memcpy_cilium(&(elem->val), val, sizeof(_name##_cuckoo_val_t));                           \
                return false;                                                                      \
            }                                                                                      \
        }                                                                                          \
                                                                                                   \
        ...
        memset_cilium(&x, 0, sizeof(struct _name##_cuckoo_hash_cell));                                    \
                                                                                                   \
        x.is_filled = true;                                                                        \
        memcpy_cilium(&x.key, key, sizeof(_name##_cuckoo_key_t));                                         \
        memcpy_cilium(&x.val, val, sizeof(_name##_cuckoo_val_t));                                         \
                                                                                                   \
        struct _name##_cuckoo_insert_loop_ctx loop_ctx;                                            \
        memset_cilium(&loop_ctx, 0, sizeof(struct _name##_cuckoo_insert_loop_ctx));                       \
        loop_ctx.map = map;                                                                        \
        loop_ctx.hash1 = hash1;                                                                    \
        loop_ctx.hash2 = hash2;                                                                    \
                                                                                                   \
        memcpy_cilium(&loop_ctx.new_elem, &x, sizeof(struct _name##_cuckoo_hash_cell));                   \
