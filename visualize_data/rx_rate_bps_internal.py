...
RX_RATE_FILE_NAME = "avg_rx_rate_bps_internal.csv"
RX_RATE_FILE_NAME_STDEV = "avg_rx_rate_bps_internal_stdev.csv"
RX_RATE_FILE_NAME_EACH_RUN = "rx_rate_bps_internal.csv"
RX_RATE_FILE_NAME_FIG = "avg_rx_rate_bps_internal.pdf"
...
def rx_rate_single_run(input_file):
    ...
    f.close()
    if rx_bps == 0:
        print(f"ERROR: no rx_bps in {input_file}. Return rx_rate = 0")
        return 0
    rx_rate_gbps = rx_bps / pow(10, 9)
    return rx_rate_gbps
...
def plot_progs_avg_rx_rate(num_cores_min, num_cores_max, input_folder, prog_name, version_name_list, version_name_show_list,
    ...
    plt.ylabel("Average internal rx rate (Gbps)", fontweight='bold')
    plt.grid()
...
def visualize_prog_avg_rx_rate_bps_internal(prog_name, version_name_list, version_name_show_list,
