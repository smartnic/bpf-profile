...
def gen_pcap_flow_affinity_portknock(dst_mac, dst_ip, output_path, input_file, pkt_len):
    print("start [gen_pcap_flow_affinity_portknock]")
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    # input_pkts = rdpcap(input_file)
    src_mac = "10:70:fd:d6:a0:1c"
    new_pkts = []
    output_file = f"{output_path}/xdp_portknock_flow_affinity.pcap"
    append_flag = False
    ...
    if new_pkts:
        wrpcap(output_file, new_pkts, append=append_flag)
    print(f"[gen_pcap_flow_affinity_portknock] output pcap: {output_file}")
...
if __name__ == '__main__':
    ...
    gen_pcap_flow_affinity_portknock(dst_mac, args.dst_ip, args.output_path, args.input_file, args.pkt_len)
