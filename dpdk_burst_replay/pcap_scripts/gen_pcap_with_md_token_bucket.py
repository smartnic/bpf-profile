...
def get_md_from_pkt(pkt):
    ...
    md_elem.time = get_md_from_pkt.static_time
    return md_elem
...
def gen_pcap_with_md_token_bucket(num_cores, dst_mac, output_path, input_file, tcp_only, pkt_len):
    global TCP_ONLY
    TCP_ONLY = tcp_only
    print(f"start [gen_pcap_with_md_token_bucket] num_cores: {num_cores}")
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    output_file = f"{output_path}/xdp_token_bucket_shared_nothing_{num_cores}.pcap"
    append_flag = False
    ...
    if new_pkts:
        wrpcap(output_file, new_pkts, append=append_flag)
    print(f"[gen_pcap_with_md_token_bucket] output pcap: {output_file}")
...
if __name__ == '__main__':
    ...
    gen_pcap_with_md_token_bucket(args.num_cores, dst_mac, args.output_path, args.input_file, args.tcp_only, args.pkt_len)
