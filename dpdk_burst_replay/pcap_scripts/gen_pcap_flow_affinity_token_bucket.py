...
def gen_pcap_flow_affinity_token_bucket(dst_mac, output_path, input_file, pkt_len):
    print("start [gen_pcap_flow_affinity_token_bucket]")
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    # input_pkts = rdpcap(input_file)
    src_mac = "10:70:fd:d6:a0:1c"
    new_pkts = []
    output_file = f"{output_path}/xdp_token_bucket_flow_affinity.pcap"
    append_flag = False
    ...
    if new_pkts:
        wrpcap(output_file, new_pkts, append=append_flag)
    print(f"[gen_pcap_flow_affinity_token_bucket] output pcap: {output_file}")
...
if __name__ == '__main__':
    ...
    gen_pcap_flow_affinity_token_bucket(dst_mac, args.output_path, args.input_file, args.pkt_len)
