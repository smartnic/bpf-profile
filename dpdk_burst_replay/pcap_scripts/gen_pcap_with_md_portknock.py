...
def gen_pcap_with_md_portknock(num_cores, dst_mac, output_path, input_file, tcp_only, pkt_len):
    global TCP_ONLY
    TCP_ONLY = tcp_only
    print(f"start [gen_pcap_with_md_portknock] num_cores: {num_cores}")
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    output_file = f"{output_path}/xdp_portknock_shared_nothing_{num_cores}.pcap"
    append_flag = False
...
if __name__ == '__main__':
    ...
    gen_pcap_with_md_portknock(args.num_cores, dst_mac, args.output_path, args.input_file, args.tcp_only, args.pkt_len)
