...
def gen_pcap_with_md_portknock_pkt_loss(num_cores, dst_mac, output_path, input_file, tcp_only, pkt_len):
    global TCP_ONLY
    TCP_ONLY = tcp_only
    print(f"start [gen_pcap_with_md_portknock_pkt_loss] num_cores: {num_cores}")
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    output_file = f"{output_path}/xdp_portknock_shared_nothing_pkt_loss_{num_cores}.pcap"
    append_flag = False
    ...
    for i, curr_pkt in read_packets(input_file):
        ...
        # print(src_mac)
        pkt_id_bytes = pkt_id.to_bytes(4, 'little')
        pkt_id += 1
    ...
    if new_pkts:
        wrpcap(output_file, new_pkts, append=append_flag)
    print(f"[gen_pcap_with_md_portknock_pkt_loss] output pcap: {output_file}")
...
if __name__ == '__main__':
    ...
    gen_pcap_with_md_portknock_pkt_loss(args.num_cores, dst_mac, args.output_path, args.input_file, args.tcp_only, args.pkt_len)
